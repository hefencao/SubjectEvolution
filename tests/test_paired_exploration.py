...
def test_functional_regulatory_oxygen_candidate_has_direct_output_checks() -> None:
    ...
    assert spec["mechanism_family"] == "functional-modules"
...
def test_functional_regulatory_candidate_manipulation_is_observable(tmp_path: Path) -> None:
    ...
    report = execute_plan(plan)
    assert report["assessment"]["manipulation_supported_seed_count"] == 2
    assert report["assessment"]["mechanism_family"] == "functional-modules"
...
def test_functional_modules_aggregate_candidate_has_direct_residual_checks() -> None:
    from se.experiments.paired_exploration import load_candidate_spec
    ...
    assert spec["primary_metric"] == "harvested-resource-total"
    assert spec["mechanism_family"] == "functional-modules"
    assert spec["family_role"] == "aggregate-path"
    ...
    assert ("baseline", "functional_module_changed_entity_fraction", ">", 0.0) in checks
    assert ("intervention", "functional_module_changed_entity_fraction", "==", 0.0) in checks
    assert ("intervention", "functional_modules_ablation_enabled", "==", 1.0) in checks


def test_functional_modules_aggregate_manipulation_is_observable(tmp_path: Path) -> None:
    from se.analysis.candidate_ledger import candidate_portfolio_metadata
    from se.experiments.paired_exploration import load_candidate_spec
    ...
    plan = build_plan(
        ...
        candidate_spec_schema=spec["schema"],
        candidate_metadata=candidate_portfolio_metadata(spec),
        output=tmp_path / "functional-aggregate-paired",
        backend="cpu",
    )
    report = execute_plan(plan)
    assert report["assessment"]["manipulation_supported_seed_count"] == 2
    assert report["assessment"]["family_role"] == "aggregate-path"
    for panel in report["panels"]:
        observed = {
            item["metric"]: item["observed"]
            for item in panel["manipulation_checks"]
            if item["branch"] == "intervention"
        }
