...
- 每 500 tick 独立输出世代深度、有效谱系、去 softmax 共同偏移的策略多样性、固定探针行为差异、窗口行动分布，以及繁殖可行—提案—接受—拒绝闭环；
...
- `evolution_progress.jsonl`：默认每 500 tick 的独立演化诊断；原始权重、规范策略、固定探针行为、世代/谱系、繁殖闭环和五类利益流必须联合解释；
- `summary.json`：最后一条指标；
