...
- 群体资源方向形成对成员行动的高层影响；
- 谱系继承、变异、出生、死亡和容量管理；
- 可版本化的繁殖容量仲裁：当前配置用无状态随机优先键消除低稳定 ID 优势，归档配置可继续回放旧排序；规则与候选/槽位审计进入出生计划和运行元数据；
- 社会依赖代理指标、群体指标、原始策略权重、信息检测率、行动熵，以及逐 step 的利益边界流、显式 H2D/D2H、实体提交字节/耗时和稠密消息字节规避统计；
- 每 500 tick 独立输出世代深度、有效谱系、去 softmax 共同偏移的策略多样性、固定探针行为差异、真实观察/行动 mask 响应、共同真实上下文策略面板、窗口行动分布，以及繁殖可行—提案—接受—拒绝闭环；
- 利益流除即时群体边界外，还按稳定实体身份冻结一个 500-tick 窗口的滞后边界；槽位复用不会把新生体误归入旧成员，五类流均保留守恒残差；
...
- `evolution_progress.jsonl`：默认每 500 tick 的独立演化诊断；原始权重、规范策略、固定探针、真实上下文响应、世代/谱系、繁殖闭环以及即时/滞后五类利益流必须联合解释；首窗滞后边界来自 tick 0 的未分组状态；
- `summary.json`：最后一条指标；
...
`entities.reproduction_capacity_arbitration` 是会改变世界轨迹的模型规则：

- `stateless-random-v1`：当前配置使用。按 run seed、tick、专用随机流和稳定实体 ID 生成优先级；候选数组顺序与物理槽位不影响结果。
- `stable-id-v1`：旧规则，按稳定实体 ID 从小到大接受，只用于归档实验回放或显式对照。

旧配置若没有该字段会默认 `stable-id-v1`，因此不会在重新运行时静默切换模型。两种结果不能混合；实际解析后的规则、候选数和可用槽位保存在 `BirthRequestPlan`，提交器会拒绝规则版本与世界配置不一致的计划，运行级规则同时写入 `config.json` 和 `run_metadata.json.model_rules`。本次修正不改变“同 tick 死亡晚于出生分配”或“容量拒绝不支付繁殖成本”。
