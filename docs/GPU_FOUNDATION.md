...
本阶段将已对照的 GPU 基础层接入运行时：无状态随机键、环境/信息场、规则网格、观察构建、参数化策略批处理和采集冲突计划。`--backend gpu` 是显式的混合模式：环境/信息场由设备权威演进，实体与社会状态目前由 CPU 提交。CPU 参考路径是语义对照，但提交执行器不永久绑定 CPU。它不是“悄悄回退”的自动优化，也不是完整设备驻留世界循环。
...
`restore-autonomy` 启用的 `independent-foraging-v1` 同样位于后端无关的主机仲裁边界。默认运行不新增传输；只有模块启用后，设备观察阶段才返回已在设备上计算的物理资源梯度，供恢复提案使用。模块的替代决策不读取消息、群体方向、伙伴或记忆，但触发条件会检查身体策略已选的行动类别；CPU/GPU 对照固定其处理队列、接管标志、行动与结果。该额外边界由模型语义触发，不作为性能迁移。
