...
本工作区已在 WSL2、NVIDIA GeForce RTX 4070、CUDA Toolkit 13.3 和 `cupy-cuda13x==14.1.1` 上验证：完整测试套件为 `41 passed`；三步 CPU/GPU 同种子场景动作计数一致，字段状态只存在小的 FP32 容差；`--ticks 16` 的字段最大绝对误差为 `2.12e-6`，小于 `5e-6` 容差。关系维护优化的已预热单 step `cProfile` 由约 `0.112秒` 降至 `0.077秒`，但 70 步短批量窗口仍受运行时波动影响；最新第20/40/60步窗口为 `0.097/0.100/0.108秒`，仅作当前机器上的规模参考。
