...
本阶段将已对照的 GPU 基础层接入运行时：无状态随机键、环境/信息场、规则网格、观察构建、演化策略批处理和采集冲突计划。`--backend gpu` 是显式的混合模式：环境/信息场由设备权威演进，实体与社会状态目前由 CPU 提交。CPU 参考路径是语义对照，但提交执行器不永久绑定 CPU。它不是“悄悄回退”的自动优化，也不是完整设备驻留世界循环。
...
| 5 | 可遗传策略矩阵、动作采样与方向选择 | 相同种子小场景动作一致；浮点状态按容差比较 |
| 6（采集子集） | `GpuActionConflictResolver` 的采集行定位、`(cell_id, entity_id)` 稳定排序和公平分配 | 只读快照输入、返回主机 `HarvestResolution`；资源只在后续设备字段提交阶段扣减 |
| 7（CPU计划边界） | `ShareResolution → RelationUpdatePlan` | 后端无关、规范事件顺序；固定关系槽仍由 CPU 提交 |
| 8（CPU计划边界） | `BirthRequestPlan → BirthAllocationPlan`、`DeathEventPlan` | 出生计划记录容量仲裁版本、候选/槽位数和稳定父 ID；槽池版本固定分配快照；死亡先记录事件、主体图更新后再统一回收槽位 |
...
设备侧：环境与信息场的权威更新、信号场发射、采集计划（稳定键排序与公平分配）、资源提交、规则网格、伙伴采样、场与伙伴观察、策略 logits/采样/方向。
...
`control.heuristic_social_guidance` 会为已采样的资源移动覆盖方向，因此只允许 `entertainment` 协议。它不读取 GPU 内部真值，也不增加设备字段或观察回传；元数据和轨迹仍记录规则及实际应用次数，但结果不得并入科学基线。

`independent-foraging-override` 的 `independent-foraging-v1` 会直接替换行动，只保留为娱乐版本；旧名 `restore-autonomy` 仅作兼容别名。默认 `scientific` 协议在执行前拒绝它；娱乐模式仍固定 CPU/GPU 的处理队列、接管标志、行动与结果，并在干预记录写入 `kind=direct-action` 和 `direct_action_control=true`。该路径用于演示/压力测试，不用于科学解释。
...
本工作区已在 WSL2、NVIDIA GeForce RTX 4070、CUDA Toolkit 13.3 和 `cupy-cuda13x==14.1.1` 上通过完整 CPU/GPU 测试，包含 136 列演化基因组的持久镜像、稀疏继承提交、周期演化评估、繁殖闭环、版本化中性容量仲裁、即时/滞后利益流守恒、真实上下文低频下载、行动一致性、环境反转、娱乐行动覆盖和共享前史分叉。三步 CPU/GPU 同种子场景动作计数一致，字段状态只存在小的 FP32 容差；`--ticks 16` 的字段最大绝对误差为 `2.12e-6`，小于 `5e-6` 容差。新增采集计划测试逐项比较稳定行/单元排序、分配、失败码，并断言设备资源在计划阶段不变；GPU 克隆测试还确认反事实分支重绑定自己的设备运行时。关系维护优化的已预热单 step `cProfile` 由约 `0.112秒` 降至 `0.077秒`；融合随机键使同一剖析中随机相关累计时间从约 `18ms` 降至 `6ms`。三通道稠密共享排序虽然在 131,072 事件微基准中为 `3.19×`，但 300 tick 端到端运行没有提升，且会阻碍异频通道；默认运行时因此采用稀疏、有序的发射计划。默认 `[1,1,1]` cadence 是不入队的直通快路径；`(1,3,2)` 异频 cadence 的端到端 CPU/GPU 对照也已通过。阶段 8.5–10 的 100k/300 tick 数字来自旧固定行为公式，只作为历史迁移记录，不能代表当前演化策略吞吐；当前 100k/5 tick 冒烟在预热后约 `0.081秒/tick`，短窗口不构成性能承诺。字段和策略观察当前仍固定为三通道。每通道传播频率或可变通道数必须作为显式模型配置实现，不能由调度器隐式跳过。

GPU→CPU 提交视图压缩现已暂停：减少传输字节没有在阶段 10 转化为端到端墙钟收益。项目先进入 M4 主体偏移；未来只有在完整剖析证明价值后，才按“行动解析与实体提交完整驻留域”恢复迁移。任何设备提交仍复用已解析、版本化计划，不能让策略核直接改写权威世界。
