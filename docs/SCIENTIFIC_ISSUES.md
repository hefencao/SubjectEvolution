...
| D2-10 | resolved as transient, guarded | Module 3 can alter flow, energy and demography, but the original persistent endpoint was not reproduced and several demographic directions reverse over time. | Re-estimate the module effect in qualified redesigned source populations before any copy-number claim. |
| D2-11 | active | Tick-zero lineage equalization can manufacture diversity if treated as an outcome. | Genotype-only unique-donor panels, natural-abundance controls and at least 600 ticks of ordinary dynamics with no lineage-aware protection; require qualification in two phases and multiple fresh seeds. |
...
| DEV-01 | guarded | Editable installs can point to another checkout or stale console metadata. | `make conda-check` must prove direct URL, exact source root, twelve entries and external smoke. |
