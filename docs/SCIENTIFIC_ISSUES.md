...
## v0.36 D1 elastic-capacity questions

| ID | 状态 | 科学价值与当前问题 | 后续执行条件 |
|---|---|---|---|
| DIFF-D1-01 | D1-A 机制完成，经验未决 | 工作记忆、知识字节、关系槽位和知识注意力已能独立遗传、限制实际机制并支付维护/发育成本；容量方差本身仍不等于分化。 | 运行 3 seed 长程，按世代和谱系报告分布、实际使用、成本、边界固定与表达中和差异。 |
| DIFF-D1-02 | 开放 | 当前初代容量基因来自同一连续初始化分布，可能使四类容量都集中在中点附近；可达边界和早期选择依赖初代方差。 | 预注册至少三档初代容量分布，不根据结果后调；报告边界堆积、灭绝偏差和有效维度。 |
| DIFF-D1-03 | 开放 | 四类结构成本均折算为能量，成本系数可能预先决定容量收缩或扩张；关系和注意力的机会成本尚未完全物理化。 | 做固定容量、零结构成本、线性成本缩放和单容量中和矩阵；区分结构成本、实际使用成本和生态收益。 |
| DIFF-D1-04 | 已有防护 | `neutralize-elastic-capacities` 同时固定四类表达，只识别联合容量表达效应，不能区分单一容量贡献。 | 长程显示机制效应后再增加单容量中和或预注册 factorial，不事后挑显著容量。 |
| DIFF-D1-05 | 开放 | 容量可能与 founder lineage、总体体力或环境位置共同漂移，简单容量—结果相关会产生伪适应解释。 | 对齐 lineage turnover、generation、alive 和环境相位；使用共同 checkpoint 表达中和与环境反转。 |
| DIFF-D1-06 | 部分实现 | D1-A 尚未覆盖传感范围、身体资源储存、质量、发育时间和生命周期内可塑容量。 | 只有四类容量表现出实际使用与选择响应后，才扩展新的物理容量；不得一次引入过多不可识别轴。 |
...
| DIFF-D1-01 | D1-A 机制完成，经验未决 | 工作记忆、知识、关系和知识注意力容量已独立遗传；传感与身体储存仍未实现。 | 以 v0.36 长程和表达中和检验实际使用与选择响应，再决定 D1-B。 |
| DIFF-D2-01 | 未实现 | 通用表达模块必须与现有 latent router 区分，否则只是重复策略网络。 | 仅连接具身物理端口，增加表达、实际使用、同源和消融证据。 |

本文件以 v0.36 代码、既有三个 seed 的 1500-tick D0 前置长跑、18-anchor manifest、108/108 event-timed pairs，以及 D1-A 容量机制与短程配对 smoke 为准。“已实现”只表示机制、诊断或防护存在，不表示科学假设成立。
