...
- 增加共享干预与自主恢复实验：CLI 可先在共同历史上执行社会切断，再从同一快照生成切断对照和 `restore-autonomy` 处理。首版 `independent-foraging-v1` 的替代决策使用本地资源、物理梯度与能量，并以身体策略已选行动类别作为触发条件；可组合 `AutonomyRecoveryArbiter` 接管符合条件的处理行，并逐贡献者记录主体 ID、控制器类型与权重。
- 恢复队列按稳定实体 ID 和无状态随机键精确抽样；处理与对照分支跟踪同一队列，指标包括存活、平均能量、累计采集、模块使用率和独立采集成功率。摘要对“未先社会切断”和“社会方向控制原本关闭”给出科学解释警告；新增 `docs/SCIENTIFIC_ISSUES.md` 登记未适合立即实现但有价值的研究问题。
