...
from ..workspace import portable_or_absolute, resolve_path, workspace_root

SCHEMA = "tiered-exploration-plan-v3"
LEGACY_SCHEMAS = {"tiered-exploration-plan-v1", "tiered-exploration-plan-v2"}
...
def _read_plan(path: Path, *, workspace: str | Path | None = None) -> dict[str, Any]:
    ...
    if payload.get("replication_protocol_sha256") is None:
        ...
        if not config_ref.is_absolute():
            candidates.extend([base / config_ref, path.parent / config_ref])
...
def load_plan(path: str | Path, *, workspace: str | Path | None = None) -> dict[str, Any]:
    ...
    return _read_plan(Path(path), workspace=workspace)
...
def build_plan(
    *,
    ...
    config_path = resolve_path(config_path, root=root)
    ...
    if required_prior:
        ...
        if stage in {"replication", "confirmation"}:
            if not prior_protocol_sha:
                raise ValueError(
                    f"{stage} requires a prior source plan with a protocol fingerprint"
                )
            if replication_protocol_sha != prior_protocol_sha:
                raise ValueError(
                    f"{stage} must preserve the source protocol exactly except for the "
                    "independent seed set; changed scale, horizon, cadence, or mechanism "
                    "settings require a separately preregistered robustness study"
                )
        prior_ref = {
    ...
    if stage == "confirmation" and not allow_large_long_confirmation:
        raise ValueError(
            "confirmation requires --authorize-confirmation (legacy alias: --allow-large-long-confirmation)"
        )
    output = resolve_path(output, root=root)
    strict_paths = workspace is not None
    run_root = portable_or_absolute(output, root=root, strict=strict_paths)
    config_ref = portable_or_absolute(config_path, root=root, strict=strict_paths)
    plan_path = f"{run_root}/exploration_plan.json"
    command = [
        "se-multi",
        "--config",
        config_ref,
    ...
    return {
        ...
        "source_protocol_locked_to_prior": bool(stage in {"replication", "confirmation"}),
        "replication_protocol_locked_to_prior": bool(stage == "replication"),
        "confirmation_protocol_locked_to_prior": bool(stage == "confirmation"),
        "stage_changes_only_independent_seeds": bool(stage in {"replication", "confirmation"}),
...
def validate_multi_seed_invocation(
    ...
    *,
    ...
    plan = _read_plan(plan_path, workspace=root)
    ...
    checks = {
        ...
        "output": output.resolve() == resolve_path(str(plan.get("run_root", plan.get("output", ""))), root=root),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--allow-large-long-confirmation", action="store_true", help=argparse.SUPPRESS)
    ...
    prior = _read_plan(resolve_path(args.prior_plan, root=root), workspace=root) if args.prior_plan else None
    plan = build_plan(
        stage=args.stage,
        ...
        prior_plan=prior,
        allow_large_long_confirmation=(args.authorize_confirmation or args.allow_large_long_confirmation),
        workspace=root,
