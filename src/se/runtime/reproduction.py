...
FIXED_CONSERVATIVE_REPRODUCTION_SCHEMA = (
...
def conservative_reproduction_investment_enabled(cfg: SimulationConfig) -> bool:
    return cfg.entities.reproduction_schema in {
        FIXED_CONSERVATIVE_REPRODUCTION_SCHEMA,
        INHERITED_REPRODUCTION_INVESTMENT_SCHEMA,
    }
...
def reproduction_investment(genotype: Any, cfg: SimulationConfig) -> Any:
    ...
    if values.ndim != 2:
        raise ValueError("genotype must be a two-dimensional entity-by-gene array")
    if cfg.entities.reproduction_schema == LEGACY_REPRODUCTION_SCHEMA:
        return xp.full(
            values.shape[0],
    ...
    if cfg.entities.reproduction_schema == FIXED_CONSERVATIVE_REPRODUCTION_SCHEMA:
        return xp.full(values.shape[0], levels[0], dtype=xp.float32)
    if values.shape[1] <= REPRODUCTION_INVESTMENT_GENE_INDEX:
...
def reproduction_energy_cost(genotype: Any, cfg: SimulationConfig) -> Any:
    ...
    if not conservative_reproduction_investment_enabled(cfg):
...
def offspring_energy_endowment(
    ...
    *,
    ...
    if conservative_reproduction_investment_enabled(cfg) and neutralized:
