...
def direct_message_transport(
    ...
    if (
        simulation.cfg.information.direct_message_propagation_schema
    ...
    for index in np.flatnonzero(valid).tolist():
        path_terrain, distance = _terrain_path_mean(
            terrain,
        ...
        terrain_factor = max(1.0 - terrain_fraction * path_terrain, 0.05)
        multipliers[index] = np.float32(
            terrain_factor * np.exp(-distance_decay * distance)
        )
