...
class Environment:
    ...
    def cell_values(self, cell_ids: np.ndarray) -> np.ndarray:
        ...
        return self.resources.reshape(4, -1)[:, cells].T.astype(np.float32)
    ...
    def gradients_for_entities(self, entity_cells: np.ndarray, capacity: int) -> tuple[tuple[np.ndarray, np.ndarray], tuple[np.ndarray, np.ndarray]]:
        ...
        rgx = np.where(cells >= 0, rgx, 0.0).astype(np.float32)
        rgy = np.where(cells >= 0, rgy, 0.0).astype(np.float32)
        hgx = np.where(cells >= 0, hgx, 0.0).astype(np.float32)
        hgy = np.where(cells >= 0, hgy, 0.0).astype(np.float32)
    ...
    def resolve_harvest(self, cell_ids: np.ndarray, rates: np.ndarray) -> np.ndarray:
        ...
        result = np.empty((cells.size, 4), dtype=np.float32)
        flat = self.resources.reshape(4, -1)
        for channel in range(4):
            available = flat[channel, cells]
            per_entity = np.minimum(requested_rates[:, channel], available / requested)
