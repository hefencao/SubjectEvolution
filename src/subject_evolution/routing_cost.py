...
def apply_routing_cost_budget(
    ...
    *,
    ...
    """
    if (
        not config.routing_cost_enabled
        or not config.latent_policy_enabled
        or plan.size == 0
    ):
        return RoutingCostBudgetResult.empty(plan)

    energy = np.asarray(active_energy, dtype=np.float64)
    unique_rows = np.unique(plan.active_rows).astype(np.int32, copy=False)
