...
test:
	$(PYTHON) scripts/run_test_shards.py --project . --shards 5 --report .validation/FINAL_TEST_REPORT.json

# Bootstrap path for CI or a clean checkout that is not installed editable.
test-src:
	PYTHONPATH=src $(PYTHON) scripts/run_test_shards.py --project . --shards 5 --report .validation/FINAL_TEST_REPORT.json
...
parity-gpu:
	rm -rf .validation/GPU_PARITY_REPORTS
	mkdir -p .validation/GPU_PARITY_REPORTS
	SE_REQUIRE_GPU_PARITY=1 SE_GPU_PARITY_REPORT_DIR=.validation/GPU_PARITY_REPORTS $(PYTHON) -m pytest -q tests/test_parity.py
	$(PYTHON) scripts/summarize_gpu_parity_reports.py --project . --reports-dir .validation/GPU_PARITY_REPORTS --output .validation/GPU_PARITY_REPORTS
...
conda-check:
	$(PYTHON) scripts/run_conda_check.py --project . --shards 5 --docs-dir .validation/conda

verify-dist:
	$(PYTHON) scripts/verify_dist.py --project . $(if $(PREVIOUS_WHEEL),--previous-wheel $(PREVIOUS_WHEEL),)

release-check:
	$(PYTHON) scripts/verify_portable_docs.py --project .
	$(PYTHON) scripts/run_release_check.py --project . --test-report .validation/FINAL_TEST_REPORT.json --report .validation/RELEASE_CHECK_REPORT.json $(if $(PREVIOUS_WHEEL),--previous-wheel $(PREVIOUS_WHEEL),)
	$(PYTHON) scripts/verify_portable_docs.py --project .
