...
test:
	$(PYTHON) scripts/run_test_shards.py --project . --shards 5 --report docs/v0.67/FINAL_TEST_REPORT.json

# Bootstrap path for CI or a clean checkout that is not installed editable.
test-src:
	PYTHONPATH=src $(PYTHON) scripts/run_test_shards.py --project . --shards 5 --report docs/v0.67/FINAL_TEST_REPORT.json
...
parity-gpu:
	rm -rf docs/v0.67/GPU_PARITY_REPORTS
	mkdir -p docs/v0.67/GPU_PARITY_REPORTS
	SE_REQUIRE_GPU_PARITY=1 SE_GPU_PARITY_REPORT_DIR=docs/v0.67/GPU_PARITY_REPORTS $(PYTHON) -m pytest -q tests/test_parity.py
	$(PYTHON) scripts/summarize_gpu_parity_reports.py --project . --reports-dir docs/v0.67/GPU_PARITY_REPORTS --output docs/v0.67
...
conda-check:
	$(PYTHON) scripts/run_conda_check.py --project . --shards 5 --docs-dir docs/v0.67

verify-dist:
	$(PYTHON) scripts/verify_dist.py --project . $(if $(PREVIOUS_WHEEL),--previous-wheel $(PREVIOUS_WHEEL),)

release-check:
	$(PYTHON) scripts/run_release_check.py --project . --test-report docs/v0.67/FINAL_TEST_REPORT.json --report docs/v0.67/RELEASE_CHECK_REPORT.json $(if $(PREVIOUS_WHEEL),--previous-wheel $(PREVIOUS_WHEEL),)
