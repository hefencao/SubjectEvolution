...
test:
	$(PYTHON) scripts/run_test_shards.py --project . --shards 5 --report docs/v0.52/FINAL_TEST_REPORT.json

# Bootstrap path for CI or a clean checkout that is not installed editable.
test-src:
	PYTHONPATH=src $(PYTHON) scripts/run_test_shards.py --project . --shards 5 --report docs/v0.52/FINAL_TEST_REPORT.json
...
conda-check:
	$(PYTHON) scripts/run_conda_check.py --project . --shards 5 --docs-dir docs/v0.52

verify-dist:
	$(PYTHON) scripts/verify_dist.py --project . $(if $(PREVIOUS_WHEEL),--previous-wheel $(PREVIOUS_WHEEL),)

release-check:
	$(PYTHON) scripts/run_release_check.py --project . --test-report docs/v0.52/FINAL_TEST_REPORT.json --report docs/v0.52/RELEASE_CHECK_REPORT.json $(if $(PREVIOUS_WHEEL),--previous-wheel $(PREVIOUS_WHEEL),)
